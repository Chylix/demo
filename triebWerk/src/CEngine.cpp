...
bool triebWerk::CEngine::Initialize(const SEngineConfiguration& a_Config)
{
	...
	result = m_pSoundEngine->Initialize(m_pResourceManager, a_Config.m_MasterVolume, a_Config.m_SFXVolume, a_Config.m_BGMVolume);
	if (!result)
		return false;
	...
	m_pTimeline->Initialize(m_pSceneManager);
...
bool triebWerk::CEngine::Run()
{
    ...
    if (frameUpdate)
    {
		...
		m_pTimeline->Update(m_pTime->GetDeltaTime(), m_pTime->GetTimeSinceStartup());
		if(!m_PausedScenes)
...
void triebWerk::CEngine::Pause(bool a_State)
{
	m_PausedScenes = a_State;
}
...
void triebWerk::CEngine::ProcessWindowMessages()
{
    ...
    for (size_t i = 0; i < messageCount; ++i)
    {
        ...
        case WM_SIZE:
        {
			...
			{
				m_pWindow->m_Width = static_cast<unsigned short>(LOWORD(msg.lParam));
				m_pWindow->m_Height = static_cast<unsigned short>(HIWORD(msg.lParam));
			}
            break;
        }

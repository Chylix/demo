...
triebWerk::CSoundSource * triebWerk::CResourceManager::GetSound(const char * a_pSoundName)
{
...
void triebWerk::CResourceManager::UpdateDefaultSoundVolumes(const float a_SFXVolume, const float a_BGMVolume)
{
	for (auto sound : m_SoundBuffer)
	{
		if (sound.second->m_SoundType == CSoundSource::ESoundType::BGM)
			sound.second->SetVolume(a_BGMVolume);
		else
			sound.second->SetVolume(a_SFXVolume);
	}
}
...
{
	size_t hash = StringHasher(RemoveFileType(a_File.FileName));
	if (ExistsResourceInBuffer(EFileType::WAV, hash))
		return;

	CSoundSource* sound = new CSoundSource;
	
	bool result = sound->LoadSoundDataFromFile(a_File.FilePath.c_str());
	if (result == false)
	{
	...
	{
		sound->m_SoundType = CSoundSource::ESoundType::SFX;
		sound->SetVolume(m_pSoundEngineHandle->GetSFXVolume());
		std::cout << "SFX" << std::endl;
	}
	else if (a_File.FilePath.find("BGM") != std::string::npos)
	{
		sound->m_SoundType = CSoundSource::ESoundType::BGM;
		sound->SetVolume(m_pSoundEngineHandle->GetBGMVolume());
		std::cout << "BGM" << std::endl;
	}
	else
	{
		sound->m_SoundType = CSoundSource::ESoundType::None;
